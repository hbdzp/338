...
public class TVRootApp extends Application {
	...
	public void onCreate() {
		...
		K_TvCommonManager.getInstance().K_disableTvosIr();
	}
	...
	public void initLanguageCfig() {
		int languageindex = K_TvCommonManager.getInstance().K_getOsdLanguage();
		Resources res = getResources();
		Configuration config = res.getConfiguration();
		if (languageindex == K_Constants.TvLanguage_ENGLISH) {
			config.locale = Locale.ENGLISH;
		} else {
			if (languageindex == K_Constants.TvLanguage_CHINESE) {
	...
	private void initLittleDownCounter() {
		...
		int value = K_TvCommonManager.getInstance().K_getOsdTimeoutInSecond();
		if (value < 1) {
			value = 5;
		}
		if (value > 30) {
