...
public class OperatorProfileListAdapter extends BaseAdapter {
	...
	private int mDtvRoute = K_Constants.TV_ROUTE_NONE;
	...
	public OperatorProfileListAdapter(Context context) {
		...
		int dtvRouteIndex = K_ChannelModel.getInstance()
				.K_getCurrentDtvRouteIndex();
		K_TvTypeInfo tvinfo = K_TvCommonManager.getInstance().K_getTvInfo();
		mDtvRoute = tvinfo.getTvTypeInfo().routePath[dtvRouteIndex];
	}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		TextView opName = (TextView) convertView
				.findViewById(R.id.oprofile_name);
		opName.setText(mOProfiles.get(position).getOperatorProfileName());
		ImageView tvSystem = (ImageView) convertView
				.findViewById(R.id.tv_system);
		...
		if (opSysType == OperatorProfileInfo.EnumDeliverySysType.E_DELIVERY_SYS_TDSD
				.ordinal()) {
			if ((mDtvRoute == K_Constants.TV_ROUTE_DVBT)
					|| (mDtvRoute == K_Constants.TV_ROUTE_DVBT2)) {
			...
			tvSystem.setImageResource(R.drawable.dvbt_focus);
				...
				.ordinal()) {
			if (mDtvRoute == K_Constants.TV_ROUTE_DVBC) {
				opName.setTextColor(Color.WHITE);
			} else {
				opName.setTextColor(Color.GRAY);
			}
