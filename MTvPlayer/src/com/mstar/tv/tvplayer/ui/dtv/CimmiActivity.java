...
public class CimmiActivity extends MstarBaseActivity {
	...
	private K_TvCiManager tvCiManager = null;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		tvCiManager = K_TvCiManager.getInstance();
		...
		cimmiListView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
				...
				tvCiManager.K_answerMenu((short) (cimmiListView
			...
			public void run() {
				while (!cimMiHasDestory) {
					...
					if (cimmiTimeCounter >= CIMMI_DELAY_TIME_COUNTER_MAX) {
						Log.i(TAG,
								"timeout, send EV_CIMMI_UI_CLOSEMMI to close cimmi");
						finish();
					}
				}
			}
	...
	public void cimmiUiDataReady() {
		...
		int mmiType = K_Constants.CIMMI_NONE;
		hasData = tvCiManager.K_isDataExisted();

		if (hasData == false) {
			return;
		}
		mmiType = tvCiManager.K_getCiMmiType();
		if (mmiType == K_Constants.CIMMI_MENU) {
			// title text
			if (tvCiManager.K_getMenuTitleLength() > 0) {
				strTitle = tvCiManager.K_getMenuTitleString();
			} else {
				strTitle = " ";
			}
			...
			// subtitle text
			if (tvCiManager.K_getMenuSubtitleLength() > 0) {
				strSubTitle = tvCiManager.K_getMenuSubtitleString();
			} else {
				strSubTitle = " ";
			}
			...
			// bottom text
			if (tvCiManager.K_getMenuBottomLength() > 0) {
				strBottom = tvCiManager.K_getMenuBottomString();
			} else {
				strBottom = " ";
			}
			...
			// selection list
			data.clear();
			for (int i = 0; i < tvCiManager.K_getMenuChoiceNumber(); i++) {
				strSelection = tvCiManager.K_getMenuSelectionString(i);
		...
		} else if (mmiType == K_Constants.CIMMI_LIST) {
			// title text
			if (tvCiManager.K_getListTitleLength() > 0) {
				strTitle = tvCiManager.K_getListTitleString();
			} else {
				strTitle = " ";
			}
			title.setText(strTitle);
			...
			// subtitle text
			if (tvCiManager.K_getListSubtitleLength() > 0) {
				strSubTitle = tvCiManager.K_getListSubtitleString();
			} else {
				strSubTitle = " ";
			}
			subTitle.setText(strSubTitle);
			...
			// bottom text
			if (tvCiManager.K_getListBottomLength() > 0) {
				strBottom = tvCiManager.K_getListBottomString();
			} else {
				strBottom = " ";
			}
			...
			// selection list
			adapter.clear();
			for (int i = 0; i < tvCiManager.K_getListChoiceNumber(); i++) {
				strSelection = tvCiManager.K_getListSelectionString(i);
				adapter.add(strSelection);
			}
			adapter.notifyDataSetChanged();
			cimmiListView.invalidate();
		} else if (mmiType == K_Constants.CIMMI_ENQ) {
			// password
			...
			// title text
			if (tvCiManager.K_getEnqLength() > 0) {
				strTitle = tvCiManager.K_getEnqString();
			} else {
				strTitle = " ";
			}
			title.setText(strTitle);
	...
	private class UiEventListener extends K_OnUiEventListener {
		...
		public boolean K_onUiEvent(int what) {
			...
			case K_Constants.TVCI_UI_DATA_READY: {
				...
				break;
			case K_Constants.TVCI_UI_CLOSEMMI: {
			...
			case K_Constants.TVCI_UI_CARD_REMOVED: {
				...
				break;
			case K_Constants.TVCI_UI_AUTOTEST_MESSAGE_SHOWN: {
				Toast toast = Toast.makeText(CimmiActivity.this,
		...
		protected void onCreate(Bundle savedInstanceState) {
			...
			editText.addTextChangedListener(new TextWatcher() {
				...
				public void onTextChanged(CharSequence s, int start,
					...
					short length = 0;
					resetCimmiTimeCounter();
					length = tvCiManager.K_getEnqAnsLength();
					// Log.i(TAG,"##length>>:" +
					// length+"=="+editText.length()+">>"+editText.getEditableText().toString());
					if (editText.length() >= length) {
						tvCiManager.K_answerEnq(editText.getEditableText()
								.toString());
